...
def generate_segments(im_orig, scale, sigma, min_size):
    ...
    #im_orig = np.dstack((im_orig, segments.astype(np.uint8)))
...
def sim_colour(r1, r2):
    ...
    col_similarity = np.sum(np.minimum(r1['color_hist'], r2['color_hist']))
...
def sim_texture(r1, r2):
    ...
    """
    tex_similarity = np.sum(np.minimum(r1['texture_hist'], r2['texture_hist']))
    #hist_r1 = calc_texture_hist(r1)
    #hist_r2 = calc_texture_hist(r2)
    #intersection = np.sum(np.minimum(hist_r1, hist_r2))
    return tex_similarity


def sim_size(size_r1, size_r2, imsize):
...
def sim_fill(size_r1, size_r2, R_r1, R_r2,imsize):
    """
...
def calc_sim(r1, r2, imsize, image):
    region_ar = image[r1["min_y"]:r1["max_y"], r1["min_x"]:r1["max_x"]]
    region_br = image[r2["min_y"]:r2["max_y"], r2["min_x"]:r2["max_x"]]

    similarity_colour = sim_colour(r1, r2)

    similarity_texture = sim_texture(r1, r2)

    size_r1 = region_ar.shape[0] * region_ar.shape[1]
    size_r2 = region_br.shape[0] * region_br.shape[1]

    similarity_size = sim_size(size_r1, size_r2, imsize)

    similarity_fill = sim_fill(size_r1, size_r2, r1, r2,  imsize)
    #print("Similarity for colour: ", similarity_colour, "Texture: ", similarity_texture, "Size: ", similarity_size, "Fill: ", similarity_fill)
        ...
        for x, pixel in enumerate(row):
            ...
            if label not in regions:
                regions[label] = {
                    ...
                    "max_x": 0, "max_y": 0, "labels": label.astype(np.uint16) #uint8 gives overflow while merge
                }
            ...
            regions[label]["size"] = (regions[label]["max_x"] - regions[label]["min_x"]) * (regions[label]["max_y"] - regions[label]["min_y"])

    #check if any region has size 0
    original_regions_size = len(regions)
    regions = {k: v for k, v in regions.items() if v["size"] > 0}
    print("Number of regions with size > 0: ", len(regions), "Original number of regions: ", original_regions_size)
    return regions
...
def selective_search(image_orig, scale=1.0, sigma=0.8, min_size=50):
    ...
    for (first, second) in neighbours:
        ...
        Similarity[(first, second)] = calc_sim(R[first], R[second], imsize, image)

    # Hierarchical search for merging similar regions
    regions_to_remove = []
    print('Original similarities:', len(Similarity), 'regions:', len(R))
    #new_R = {}
    while Similarity != {}:
        # Get highest similarity
        i, j = sorted(Similarity.items(), key=lambda i: i[1])[-1][0]

        # Task 4: Merge corresponding regions. Refer to function "merge_regions"
        t = max(R.keys()) + 1.0
        #print('Merging', i, j, 'with similarity', Similarity[(i, j)])
        R[t] = merge_regions(R[i], R[j])
        #new_R[t] = R[t]
        # Task 5: Mark similarities for regions to be removed
        ### YOUR CODE HERE ###

        regions_to_remove = [] #make it empty in each iterations
        #Iterate over each item in the dictionary S
        for region_pair in Similarity.keys():
            #Check if either i or j is in the current region pair
            if i in region_pair or j in region_pair:
                #append the region pair to the list
                regions_to_remove.append(region_pair)


        # Task 6: Remove old similarities of related regions
        ### YOUR CODE HERE ###
        for rel_regions in regions_to_remove:
            del Similarity[rel_regions]
        ...
        for k in regions_to_remove:
            if k != (i, j):
                ...
                Similarity[(t, n)] = calc_sim(R[t], R[n], imsize, image)
                            #calc_sim(first, second, imsize, image, R)

    # Task 8: Generating the final regions from R
    regions = []
    ### YOUR CODE HERE ###
    print('Final similarities:', len(Similarity), 'regions:', len(R))
    regions = [{
...
#Question 5.1.1
"""
Felzenszwalb's algo is generating segments but each segment necessarily doesn't represent the object we may have to join multiple segments to join the one object that why we need selective search.
it follows the hierarchy which allowing us to understand the regions at different level of granularity
"""
#Question 5.1.2
"""
criteria 1
excluding same rectangle (with different segments)
effect: prevents the algorithm from considering the same region multiple times


criteria 2
excluding smaller regions
effect: removes very small regions and uninterested regions to remove noise

criteria 3
# excluding distorted rects
effect: Excludes rectangles where the aspect ratio is too distorted
"""
#Question 5.1.3
"""
after merging we make a bounding boxes aroud them by calculating minimum x coordinate
and y coordinate and maximum x coor and max y coor
"""
